...
export function Shipping() {
  ...
  return (
    <div className='overflow-y-scroll pb-4'>
      <h2 className='text-2xl font-medium py-2 border-b-2'>SHIPPING INFORMATION</h2>
      <form onSubmit={handleSubmit((data) => updateShippingInfo(data))}>
        ...
        <div>
          <div>
            {shippingMethods.map((method) => {
              return (
                <label key={method.name} className='flex gap-4'>
                  <input
                    {...register('selectedShipping', { required: 'This field is required.' })}
                    type='radio'
                    value={method.name}
            ...
            <p className='text-red-500 my-1'>{errors['selectedShipping']?.message}</p>
          </div>
        </div>
